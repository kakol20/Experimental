...
(function() {
                    ...
                    if (!isIE && typeof symbol.codes != "undefined") {
                        for (i = 0; i < result[0].childNodes.length; i++)
                            if (result[0].childNodes[i].nodeName == "mi" || result[0].nodeName == "mi") {
                                ...
                                else result[0].replaceChild(createMmlNode("mo").appendChild(document.createTextNode(newst)), result[0].childNodes[i]);
                            }
                    }
                    node = createMmlNode(symbol.tag, result[0]);
    ...
    function AMautomathrec(str) {
        ...
        var secondenglishAMtoken = "|\\bI\\b|\\bin\\b|\\btext\\b";
        var simpleAMtoken = "NN|ZZ|QQ|RR|CC|TT|AA|EE|sqrt|dx|dy|dz|dt|xx|vv|uu|nn|bb|cc|csc|cot|alpha|beta|delta|Delta|epsilon|gamma|Gamma|kappa|lambda|Lambda|omega|phi|Phi|Pi|psi|Psi|rho|sigma|Sigma|tau|theta|Theta|xi|Xi|zeta";
        ...
        var re2 = new RegExp("(^|\\s)([a-z]|" + texcommand + ambigAMtoken + simpleAMtoken + ")([,.])", "g");
        ...
        str = str.replace(/((^|\s)\([a-zA-Z]{2,}.*?)\)`/g, "$1`)");
        str = str.replace(/`(\((a\s|in\s))(.*?[a-zA-Z]{2,}\))/g, "$1`$3");
    ...
    function processNodeR(n, linebreaks, latex) {
        ...
        if (n.childNodes.length == 0) {
            if ((n.nodeType != 8 || linebreaks) && n.parentNode.nodeName != "form" && n.parentNode.nodeName != "FORM" && n.parentNode.nodeName != "textarea" && n.parentNode.nodeName != "TEXTAREA") {
                str = n.nodeValue;
                if (!(str == null)) {
                    str = str.replace(/\r\n\r\n/g, "\n\n");
